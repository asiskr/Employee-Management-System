...
class Student:
    def __init__(self, root):
        ...
        self.root.title("STUDENT DETAIL SYSTEM")
    ...
    def create_widgets(self):
        # Main frame
        self.main_frame = Frame(self.root, bd=2, bg=self.bg_color)
    ...
    def create_left_frame(self):
        # Left frame for student details and photo options
        left_frame = LabelFrame(self.main_frame, bd=2, bg=self.bg_color, relief=RIDGE, text="STUDENT DETAILS", font=("Arial", 12, "bold"))
        ...
        for i, label_text in enumerate(student_info_labels):
            label = Label(left_frame, text=label_text, font=("Arial", 12, "bold"), bg=self.bg_color, fg=self.text_color)
        ...
        take_photo_radio = Radiobutton(left_frame, text="Take Photo Sample", variable=self.take_photo_var, value=True, font=("Arial", 12), bg=self.bg_color, fg=self.text_color)
        take_photo_radio.grid(row=len(student_info_labels), column=0, columnspan=2, padx=10, pady=5, sticky=W)

        no_photo_radio = Radiobutton(left_frame, text="No Photo Sample", variable=self.take_photo_var, value=False, font=("Arial", 12), bg=self.bg_color, fg=self.text_color)
        no_photo_radio.grid(row=len(student_info_labels) + 1, column=0, columnspan=2, padx=10, pady=5, sticky=W)

    def create_right_frame(self):
        # Right frame for current course information and search system
        right_frame = LabelFrame(self.main_frame, bd=2, bg=self.bg_color, relief=RIDGE, text="CURRENT COURSE INFORMATION", font=("Arial", 12, "bold"))
        ...
        for i, label_text in enumerate(current_course_labels):
            label = Label(right_frame, text=label_text, font=("Arial", 12, "bold"), bg=self.bg_color, fg=self.text_color)
            label.grid(row=i, column=0, padx=10, pady=5, sticky=W)
        ...
        search_criteria_label = Label(right_frame, text="Search by:", font=("Arial", 12, "bold"), bg=self.bg_color, fg=self.text_color)
        ...
        submit_button = Button(right_frame, text="Submit", command=self.search_student, font=("Arial", 12), bg=self.button_color, fg=self.button_text_color)
        submit_button.grid(row=len(current_course_labels) + 1, column=0, padx=10, pady=5, sticky=W)

        show_all_button = Button(right_frame, text="Show All", command=self.show_all_students, font=("Arial", 12), bg=self.button_color, fg=self.button_text_color)
        show_all_button.grid(row=len(current_course_labels) + 1, column=1, padx=10, pady=5, sticky=W)

    def create_table_frame(self):
        # Table frame with scrollbars
        table_frame = Frame(self.main_frame, bd=2, bg="white", relief=RIDGE)
